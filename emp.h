	...
	switch(i){
		...
		case '4':{
			...
			do{
				...
				system("cls");
				line_end(6);
				cout<<setw(100)<<"Enter 'A' to see the list of all employees."<<endl<<endl;
				cout<<setw(110)<<"Enter the employee's number whose profile is to be deleted :: ";
				color(14);
				cin>>ws;
				cin>>del;
				if(del=="a"||del=="A"){
...
void employee::emp_input(employee emp[])
{
	...
	do{
		...
		if(strlen(emp_num)!=10)
			...
			for(int j=0;j<strlen(emp_num);j++){
				if(!isdigit(emp_num[j])){
					check=false;
					break;
				}
			}
...
void employee::emp_modify(employee emp[])
{
	...
	do{
		...
		check=number(temp_num);
		if(!strcmp("999",temp_num)){
			cout<<"strcmp";
			check=true;
		}
		if(check==false)
	...
	if(strcmp(temp_num,"999"))
		strcpy(emp_num,temp_num);
}
